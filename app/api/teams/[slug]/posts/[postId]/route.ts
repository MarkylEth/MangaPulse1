// app/api/teams/[slug]/posts/route.ts
...
export async function GET(req: NextRequest, { params }: Params) {
  try {
    ...
    const uid = await getViewerId(req) // может быть null
    ...
    const r = await query<{
      ...
      select
        ...
        case when $4::uuid is not null and exists(
          ...
          where l.post_id::text = b.id and l.user_id = $4::uuid and l.is_like = true
...
export async function POST(req: NextRequest, { params }: Params) {
  try {
    ...
    const member = await isTeamMember(team.id, uid)
    if (!member) return NextResponse.json({ error: 'Forbidden' }, { status: 403 })

    const payload = await req.json().catch(() => ({} as any))
    
    // Поддерживаем оба варианта: content и body
